...
import {
  studioContentSubmissionFrameworkMeta,
  studioContentSubmissionFrameworkSections,
} from '../content/studioContentSubmissionFramework';
import { useLocale } from '../i18n/useLocale';

function StudioContentSubmissionFramework() {
          ...
          <motion.div variants={fadeInUp} className="max-w-4xl">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass mb-6">
              ...
              <span className="text-sm text-gray-600">
                {t('policies.studioContentSubmissionFramework')}
              </span>
            </div>
            ...
            <h1 className="text-5xl sm:text-6xl font-bold tracking-tight mb-6 text-gray-900">
              {isRTL ? (
                <>
                  <span className="gradient-text text-glow">محتوى الاستوديو</span> إطار تقديم
                </>
              ) : (
                <>
                  Studio Content Submission <span className="gradient-text text-glow">Framework</span>
                </>
              )}
            </h1>

            <p className="text-lg text-gray-500 leading-relaxed max-w-3xl">
              {pick(studioContentSubmissionFrameworkMeta.summary)}
            </p>
          </motion.div>
        </div>
      </section>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="rounded-[2rem] border border-gray-200 bg-white shadow-sm overflow-hidden">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                  ...
                  <p className="text-lg font-semibold text-gray-900">
                    {studioContentSubmissionFrameworkMeta.reference}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-gray-400 uppercase tracking-[0.2em] font-semibold">
                  ...
                  <p className="text-lg font-semibold text-gray-900">
                    {pick(studioContentSubmissionFrameworkMeta.effectiveDate)}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="px-8 py-10 space-y-10">
              {studioContentSubmissionFrameworkSections.map((section) => (
              ...
              <div className="rounded-3xl border border-primary-100 bg-primary-50/60 px-6 py-5">
                <p className="text-sm text-gray-600 leading-7">
                  {isRTL
                    ? `للاستفسار عن هذا الإطار أو متطلبات تقديم محتوى الاستوديو، يرجى التواصل معنا عبر قنوات الدعم الرسمية الخاصة بـ ${brandName} أو من خلال صفحة `
                    : `For questions about this framework or studio content submission requirements, please contact us through the official ${brandName} support channels or use the `}
...
export default StudioContentSubmissionFramework;
