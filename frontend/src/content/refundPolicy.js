...
export const refundPolicySections = [
  ...
  },
  {
    ...
    bullets: [
      ...
      text('Each successful renewal is treated as a new subscription billing transaction.', 'تُعامل كل عملية تجديد ناجحة على أنها معاملة فوترة اشتراك جديدة.'),
...
export const CHECKOUT_DISCLAIMER_PARAGRAPHS = [
  ...
  text('You further understand that eligible subscription products on Aiqda may renew automatically using the saved payment method unless you turn off auto-renew before the renewal date.', 'كما تدرك أن منتجات الاشتراك المؤهلة على Aiqda قد يتم تجديدها تلقائيًا باستخدام وسيلة الدفع المحفوظة ما لم تقم بإيقاف التجديد التلقائي قبل تاريخ التجديد.'),
